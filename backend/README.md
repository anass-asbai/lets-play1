...
Product creation also accepts the optional `category` field.
...
- `POST /users` creates a user. The optional `role` field can be assigned by an administrator.
- `PUT /users/{id}` updates name, email, password, or role.
- `DELETE /users/{id}` deletes the user and their products.
...
>>>>>>> ce4de33 (Complete user CRUD and product categories)
